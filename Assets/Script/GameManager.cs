...
public class GameManager : MonoBehaviour
{
        ...
        if(Input.GetMouseButton(0) || (Input.touchCount == 1 && Input.GetTouch(0).phase == TouchPhase.Moved))
        {
            ...
            if (wait)
            {
                ...
                if (move)
                {
                    ...
                    if(k == 0)
                    {
                        //가로, 세로 같은 블럭이 없으면 ㅣ이 0이 되어서 게임오버
                        for (y = 0; y <= 3; y++)
                            for (x = 0; x <= 2; x++)
    ...
    //[x1, y1] 이동 전 좌표, [x2, y2] 이동될 좌표
    void MoveOrCombine(int x1, int y1, int x2, int y2)
    {
        //이동 될 좌표 비어있고, 이동 전 좌표에 존재하면 이동
    ...
    void Spawn()
    {
        ...
        Square[x, y] = Instantiate(Random.Range(0, int.Parse(Score.text) > 800 ? 4 : 8) > 0 ? n[0] : n[1], new Vector3(-1.8f + 1.2f * x, -1.8f + 1.2f * y,0), Quaternion.identity); //1/8확률로 4 spawn
